...
func (s *Server) handleFixAllCodeAction(ctx context.Context, uri lsproto.DocumentUri) (lsproto.CodeActionResponse, error) {
	...
	workspace := s.newSpeculativeLintWorkspace(uri, originalContent, snapshot)
	...
	if err != nil {
		// Preserve the established LSP policy: a later observation failure
		// returns content committed by earlier request-local rounds.
		log.Printf("Error running lint for fixAll: %v", err)
	}
	currentContent := workspace.currentContent
