...
func configuredDocumentPipelineResultForTest(
	...
	return linter.RunPipeline(ctx, linter.NewLintRequest(
		&documentLintSource{server: s, uri: uri, snapshot: snapshot},
...
func runSpeculativeFixAllForTest(
	...
	t.Helper()
	workspace := s.newSpeculativeLintWorkspace(uri, original, snapshot)
	if workspace.environment.baseFS == nil {
		workspace.environment.baseFS = osvfs.FS()
	}
