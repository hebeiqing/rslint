...
| `cmd/rslint/`                  | Main Go binary entry point with CLI, API, and LSP modes                                                                | Owns mode selection and process stdio/exit-code composition. The default CLI assembles config, target, Program-loader, physical-write, plugin-transport, and output adapters around the single `internal/linter.RunPipeline` use case; `--api` delegates concrete requests to `internal/api/server`; `--lsp` delegates to `internal/lsp`                                                                                                                                  |
...
| `internal/linter/`             | Unified lint pipeline, core engine, plugin dispatch, traversal, and bounded fix lifecycle                              | Owns the sole `RunPipeline` use case over immutable `GenerationProvider` / `AutofixWorkspace` ports: generation leases, plan preparation, native/plugin scheduling, stable target projection, pure fix planning, bounded re-observation, and rule aggregation. Raw stages remain internal implementation capabilities that CLI/API/LSP do not coordinate. The package imports neither config discovery, Program loaders, persistence media, output, nor API/LSP protocols |
| `internal/lsp/`                | Language Server Protocol implementation                                                                                | Wraps `typescript-go project.Session`, owns transactional config discovery and last-good commit state with `packages/vscode-extension` as the module/plugin host, and implements document-generation, speculative-workspace, reverse-plugin, publication, cancellation, and stale-generation adapters around `internal/linter`; it owns no lint execution facade                                                                                                          |
...
4. **Immutable Execution Generation**: each integration implements `linter.GenerationProvider` and freezes its current Programs, exact target projection, generation-local rule resolver, plugin projection, and absolute source-to-target identity into `linter.Generation`. `RunPipeline` acquires that value with a separate exact-once `ReleaseFunc`; callers cannot release integration-owned Program state while lazy native reads are possible, and detached plugin work is fully frozen before release. The release is normally a no-op for request-scoped CLI/API state and composes with the watcher finalizer on LSP session-external Programs. Config discovery, loader/session selection, protocol generations, and persistence never move into the linter.
5. **Semantic Analysis**: `RunPipeline` privately prepares one immutable plan, reads each Program's per-file checker capability once, freezes that result with the file's configured rules, and acquires a checker only for eligible files. LSP can additionally narrow rule eligibility while constructing its generation. Program capability, configured-rule eligibility, actual checker delivery, and program-wide diagnostics remain separate decisions.
6. **Rule Registration**: Enabled rules register listeners keyed by AST kind.
7. **AST Traversal**: The linter traverses each file once using a DFS walk. It prunes syntax that TypeScript-Go synthesized from JSDoc comments; ESLint-compatible parsers expose that text through comment APIs rather than rule AST listeners.
8. **Rule Execution**: Listener callbacks inspect nodes and may use syntax only or syntax plus type information.
9. **Diagnostic Collection**: Findings are reported as `RuleDiagnostic` values, with optional fixes or suggestions. CLI/API choose a joined plugin policy on their sealed pipeline request, which starts fully materialized plugin work before native execution and always cancels/joins it before returning. LSP diagnostics use `NewProgressiveLintRequest` with one request-scoped `ProgressiveDiagnostics` port. `RunPipeline` privately freezes enrichment, releases the generation, synchronously publishes the complete baseline, applies the syntax/no-work gate, and only then submits a detached one-shot run. The LSP port implementation owns timeout, cancellation, generation-stamped transport, and document/config-generation admission, but it cannot select or reorder lint stages.
10. **Output Generation**: CLI builds one report from the final post-fix diagnostics and passes it to `internal/output`; API returns structured data, and LSP converts diagnostics to LSP diagnostics/code actions.
...
Generation and autofix failures follow explicit lifecycle contracts:

- after a successful `AcquireGeneration`, `RunPipeline` wraps the returned
  `ReleaseFunc` immediately and invokes it exactly once on success, error, or
  a panic unwinding through the pipeline goroutine; acquisition failures remain
  the provider's cleanup responsibility
- cancellation is cooperative at preparation, native execution, fix-source
  freezing, fix planning, apply, and re-observation boundaries. The native
  engine cannot be interrupted in the middle of a rule run
- concurrent joined mode cancels and joins plugin work before returning after
  native failure or cancellation, and also during a native panic unwind. A
  plugin dispatcher must honor its context; if it does not, the pipeline waits
  rather than release a generation still reachable by joined work. After-native
  joined and progressive modes freeze plugin wire/config/text data first and
  release the generation. A submitted `DeferredPluginRun` contains no Program,
  SourceFile, VFS, dispatcher, or release capability; the progressive port
  injects transport only inside its owned asynchronous lifecycle
- plugin transport status remains a structured outcome for each integration's
  presentation policy; rebuilt plugin diagnostics still join the same result
- `ApplyChanges` may confirm a strict subset of planned paths together with an
  error for a non-atomic medium. The coordinator validates uniqueness and
  subset/exact-set rules and never observes a later generation after such an
  error
- `AppliedFixResult.Verified` is true exactly when its last observation
  describes the workspace's current state. A possible partial apply, a
  committed generation followed by lint/cancellation failure, or a final
  unobserved commit leaves it false
...
`internal/linter` owns both producer semantics and their single execution use
case. `RunPipeline` collects target syntax diagnostics, prepares the
native/plugin rule plan once per generation, collects native diagnostics safely
across workers, joins or detaches plugin work according to the sealed request,
and projects every producer into the generation's stable absolute fix-target
identity. Joined CLI/API producers enter integrations through one tagged
`PipelineResult`; progressive LSP enrichment enters through the core-submitted,
frozen one-shot continuation after its baseline result. Callers cannot invoke a
public native stage as a competing pipeline. TypeScript program diagnostics
retain their richer message-chain and related-information formatting. Completed CLI and API diagnostic sets are
stably ordered by caller-visible file path and start byte offset only after each
surface has projected absolute target paths into its presentation space; equal
keys retain producer emission order. LSP deliberately does not use that
completed-set ordering because it publishes native results first and merges an
opaque, generation-stamped plugin continuation later.
...
bases, stderr notices, plugin failure policy, and protocol empty-array rules do
not belong to the shared pipeline. The bounded fix state machine does belong
there, but its explicit policy preserves intentional surface behavior and its
`AutofixWorkspace` port leaves persistence semantics to each integration while
binding observation and mutation to one coherent source lineage.
...
Low-level edit conflict resolution remains in
...
The private planner inside `internal/linter.RunPipeline` lifts that operation to
a deterministic pure whole-file change set. Its inputs are generation-frozen
backing strings (with the BOM included when the medium carries one) and
diagnostics from that same observation. It never reads a live filesystem,
writes a file, updates an overlay, expands aliases, invalidates Programs, or
projects a protocol path.

The bounded autofix coordinator measures its safety cap in fix rounds, not lint
passes or files written. A round is one attempt to commit a non-empty planned
change set; it may contain many files, and an applied same-text fix still counts
because application—not byte inequality—is the compatibility signal.
`MaxFixRounds = 10` is the product-wide ceiling. One `AutofixWorkspace` owns the
coherent lineage through `AcquireGeneration` and `ApplyChanges`; the sealed
autofix request prevents callers from pairing an observer with an unrelated
medium. A verification observation never consumes a round and never persists
its fixes. `ApplyResult` confirms exact target paths only; the linter validates
that set against its immutable plan and derives applied diagnostic counts
instead of trusting storage-owned counters.

The CLI disk session tries independent files in stable order, refuses a change
whose freshly decoded disk representation no longer matches the generation's
frozen `Before` value, and invalidates the complete loader source generation
after every non-empty apply attempt, including partial failures. The comparison
preserves the decoded BOM representation used by the fixer; it is an optimistic
stale-write guard rather than filesystem transaction isolation. The LSP session
advances only request-local speculative text, so discarding it is rollback and
the real Session/document state is never mutated. The current API plans one
request-local output change set without committing it; the JavaScript host
remains responsible for writing the returned output.
...
- **CLI `--fix`**: requests native autofixes, permits at most ten fix rounds,
  and uses one non-persisting verification observation after the final allowed
  round. Native verification demand is `None`; plugin fix artifacts remain
  materialized for compatibility but are not applied
- **LSP quick fix**: returns direct text edits for one diagnostic
- **LSP fix-all**: uses the same bounded coordinator over a speculative-memory
  session, preserves its whole-operation plugin timeout and cycle policy, and
  returns one whole-document replacement edit
...
- **LSP speculative fix-all observations**: request native autofixes only
- **API**: `lint({ fix: true })` plans fixes from a single observation and returns the fixed source per file in `output` (the JS side persists it via `Rslint.outputFixes`). There is no separate `applyFixes`, and—unlike the CLI—it does not re-observe a committed generation.
...
catalog, and declared projects. `lint_source.go`, `lint_project_selection.go`,
and `lint_overlay.go` adapt session or isolated-overlay Programs into linter
generations; `lint_workspace.go` adapts request-local fix text. `diagnostics.go`
and `code_action.go` consume the same snapshot and call `linter.RunPipeline`
directly, while `eslint_plugin.go` owns only reverse transport and
generation-admitted continuation delivery. These files form one protocol
integration path, not a second lint execution service.
...
The default Go CLI mode keeps one command handler in `lint_command.go`. Supporting files own bounded command-layer concerns: `lint_options.go` parses invocation arguments and validates flag combinations; `lint_configuration.go` clones request-owned config maps and renders catalog warnings; `lint_diagnostics.go` normalizes diagnostics and explicit-file warnings; `lint_source.go` adapts loader generations to `linter.GenerationProvider`; `lint_fix.go` implements the physical-disk `AutofixWorkspace`; and `eslint_plugin.go` supplies the CLI's owner-directory plugin routing projection and dispatch-failure stderr policy. Target selection remains in `internal/config/target`, source-to-config rule resolution in `internal/config/lint`, Program construction and active-owner project filtering in `internal/program/loader`, the sole execution and bounded fix lifecycle in `internal/linter.RunPipeline`, and report rendering in `internal/output`.

The Go API mode keeps one request handler in `internal/api/server/lint.go` and one immutable source adapter in `lint_source.go`. Its `Handler` binds lint and AST requests to reverse plugin/config transports; bounded files own reverse config loading and activation, request-scoped overlay and canonical-path projection, API plugin routing/error presentation, and the separate inspector request with its content/options-keyed process cache. `cmd/rslint/api.go` only binds that handler to the generic `internal/api.Service` over process stdio and translates service failure to an exit code. API-specific path bases, opaque plugin routing keys, structured responses, and its current single-observation output policy remain inside `internal/api/server`; syntax/plan/native/plugin composition and pure whole-file fix planning belong to `internal/linter.RunPipeline`.
...
7. **Generation Acquisition**: each operation passes a `GenerationProvider` (or one `AutofixWorkspace`) to a sealed linter request. The provider returns one ordered sequence of rslint Programs, every selected target, a generation-local rule handler, target/fix-text projection, optional plugin projection, and an exact-once release function. CLI and native API back the rule handler with the same `internal/config/lint` resolver and stable source-to-target binding; LSP uses its frozen single-document snapshot. Providers never receive observation or round numbers and never call lint stages.
8. **Rule Execution**: `RunPipeline` privately prepares the immutable `LintPlan` and invokes `RunLinter()` over that exact plan. It asks only the Program facade to acquire checkers, retaining checker-exclusive grouping when available and sharding sufficiently large checker-free generations across at most `GOMAXPROCS` workers. Small checker-free Programs stay in one shard so scheduling and shared-consumer overhead cannot dominate their rule work. Each Program constructor freezes its complete source universe; target/config filters produce a separate execution projection, while cross-file rules always see the full universe. Plans bind the exact Program pointer, so a reparse/fix generation must construct a new Program, resolver, and plan. Module graphs and other derived structures are cached by Program generation rather than copied into `RuleContext`. Native edit demand remains independent of rule selection. When `--type-check` is enabled, Phase 2 schedules only Programs that expose complete program diagnostics; source-only generations expose no such capability and require no parallel skip state.
9. **Result Aggregation**: CLI/API choose concurrent joined plugin execution and receive one completed tagged result; plugin transport failures and recoverable protocol notices remain structured outcomes for surface-owned presentation. LSP diagnostics pass one request-scoped `ProgressiveDiagnostics` implementation. The core fixes release → baseline presentation → eligibility → enrichment submission order; the adapter injects its generation-stamped dispatcher and admits an asynchronous increment only for the still-open matching document generation. No integration builds plugin inputs, invokes raw native/plugin stages, or extracts an executable pipeline phase from the result.
10. **Fix Lifecycle**: `RunPipeline` alternates immutable observations with the workspace's medium-specific `ApplyChanges`. CLI rebinds the unchanged target plan after each confirmed round; a file may be owned by a different Program generation after its import graph changes, so Program objects are never refreshed in place. The safety bound is `MaxFixRounds = 10` non-empty change-set commit attempts, with one additional non-persisting observation only for policies that request final verification. API fix is a sealed plan-once request and never mutates its overlay; LSP fix-all advances only a request-local speculative workspace.
...
- `Rslint.lintFiles()` applies the same rule before creating a third-party
  ...
  work.
- LSP uses different Session, overlay-storage, progressive-presentation, and
  stale-admission adapters while the same linter pipeline remains the sole
  orchestrator. Session access stays on its main dispatch loop. Its
  Program-source lookup follows the same exact lexical and canonical filesystem
  identity rules as CLI/API binding, including file-symlink aliases and
  rejection of case-folded nonidentical paths.
...
- **Buffered Diagnostic Collection**: parallel shared generation execution funnels native diagnostics through a buffered channel, while single-threaded/LSP document execution appends directly without a collector goroutine
...
- **Generation-Based Fix Invalidation**: after every non-empty CLI disk-write attempt, including one that partially or completely reports failure, the compiler host atomically installs an empty source generation before any possible Program rebuild. Swapping generations rather than clearing a live map prevents an older in-flight read from repopulating the new generation. The API fix path only returns a pure planned output change set and does not mutate or rebuild its overlay, while the LSP fix session advances request-local speculative text and never mutates its long-lived Session.
...
- **Bounded Multi-Round Fixing**: `internal/linter.RunPipeline` counts non-empty planned change-set commit attempts explicitly and caps CLI `--fix` and LSP `fixAll` at `MaxFixRounds = 10`; one round may attempt many files and an applied same-text fix still consumes a round. CLI requests one final non-persisting observation after round ten; the current LSP compatibility policy does not. The API remains a one-observation output plan until its product semantics are migrated separately.
...
- **Bounded Queues**: parallel lint generations use a diagnostic channel of 4096 items; LSP request/outgoing queues are buffered to 100, and debounce/refresh signals are single-slot channels
...
├───────────────────────────────────────────────────────────┤
│ Unified Lint Pipeline / Engine / Autofix Lifecycle        │  ← internal/linter/, internal/rules/,
│                                                           │     internal/plugins/
├───────────────────────────────────────────────────────────┤
...
- **Pipeline Boundary**: `internal/linter.RunPipeline` is the sole lint use case. The linter may depend on Program and rule semantics but never on config discovery, `program/loader`, OS/VFS persistence, API/LSP protocols, or output. Integrations implement only `GenerationProvider`, `AutofixWorkspace`, plugin-dispatch, `ProgressiveDiagnostics`, and final result-projection/admission adapters. An AST architecture test admits exactly one static call in the CLI command, API handler, LSP diagnostics, and LSP fix-all operations; requires the LSP diagnostics operation to construct its progressive request directly; rejects raw-stage access from integrations; and rejects integration, persistence, and presentation imports from the linter.
...
│     │                                                                        │
│     ▼                                                                        │
│  Frozen document/config generation -> rslint Program                         │
│     │                                                                        │
│     ▼                                                                        │
│  linter.RunPipeline(document GenerationProvider)                            │
│     ├── ProgressiveDiagnostics: baseline -> admitted async enrichment       │
│     └── fixAll -> speculative AutofixWorkspace -> bounded re-observe        │
│                       │                                                      │
│                       ▼                                                      │
...
- **Lint Generation**: Immutable execution input containing one coherent Program set, target projection, and generation-local rule resolver. A lease keeps any integration-owned backing state alive until native reads finish; source changes always require another generation.
