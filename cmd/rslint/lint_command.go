...
func handleLintCommand(args lintArgs, ctx context.Context, dispatch linter.EslintPluginDispatcher) int {
	...
	generationForBinding := func(binding loader.LoadResult) linter.Generation {
		...
		targetPath := func(sourcePath string) string {
			if lintTarget, ok := target.LookupSourceTarget(binding.LintTargetBySourcePath, sourcePath, fs); ok {
		...
		return linter.Generation{
			Native: linter.NativeGeneration{
				...
				Targets:        binding.TargetsByProgram,
	...
	if fix {
		workspace := &cliAutofixWorkspace{
			cliGenerationProvider: provider,
			sourceGeneration:      programSession,
		}
