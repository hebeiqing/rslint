...
func parseLintFlags(argv []string) (args lintArgs, help bool, fatalExitCode int) {
	...
	if positional := fs.Args(); len(positional) > 0 {
		for _, arg := range positional {
			...
			// Alias edge cases are resolved once by target planning and the
			// Program file index, then frozen into the exact TargetFiles binding;
			// the linter does not consult the physical filesystem.
